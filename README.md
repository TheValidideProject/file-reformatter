...
Run the Flask server to use a web interface for converting images. After starting the server, open `http://localhost:5000` and drop one or more `.heic`/`.heif` files onto the page. The browser will automatically download the converted file (or a ZIP archive when uploading multiple images).
...
2. Install the system library required by `pyheif`:
...
5. In the **Ports** tab, expose port 5000 and open it in the browser.
6. Drag and drop `.heic` or `.heif` files onto the page to download the converted PNG images.
